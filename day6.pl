...
sub solution_1 {
    ...
    my @chunks = split(/\n\n/, $i); # split blank lines
    my $total = 0;
    for my $chunk (@chunks) { # loop on each group
        my $merged_lines = $chunk =~ s/\n//gr; # non destructive substitution
        my @chars = split(//, $merged_lines); # split chars to get char array
        my %unique = map { $_ => 1 } @chars; # create a map {char1 => 1, char2 => 1...} https://perlmaven.com/unique-values-in-an-array-in-perl
        my $count = keys %unique; # so we get unique keys = unique chars
        $total += $count;
    }
   return $total;
}

# print "total: ", solution_1($input),"\n";
...
sub solution_2 {
    ...
    my $total = 0;
    for my $chunk (@chunks) {
        my $group_size = split(/\n/, $chunk); # length of splitted chunk by line break
        my $merged_lines = $chunk =~ s/\n//gr;
        my @chars = split(//, $merged_lines);
        my %count_map = map { $_ => 0 } @chars; # init map of unique chars
        for my $char (@chars) {
            $count_map{$char} += 1; # loop over chars and increment de count map
        }
        for my $char (keys %count_map) {
            if ($count_map{$char} == $group_size) { 
                $total+=1; # increment total if count of that question = group size
            }
        }
    }
   return $total;
