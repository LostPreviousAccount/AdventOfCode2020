...
# parsing en 4 cols : min max char password
# EN R, ON COMPTE A PARTIR DE 1
parseline = function(line) { strsplit(line, " ") } 
var = lapply(strsplit(input, "\n")[[1]], parseline) # découpe ligne par ligne puis par espace
pass = sapply(var, function(v) {v[[1]][[3]]}) # car var est une liste de liste, on doit se taper le [[1]]..
char = sapply(var, function(v) {substr(v[[1]][[2]], 1, 1)}) 
minmax = lapply(var, function(v) { strsplit(v[[1]][[1]], "-")} )
min = as.numeric(sapply(minmax, function(mm) {mm[[1]][[1]][1]}))
max = as.numeric(sapply(minmax, function(mm) {mm[[1]][[2]][1]}))

#df (c'est presque lisible à partir de maintenant)
df = data.frame(min, max, char, pass)

# partie 1
# fonction pour check validité password
countchar = function(line) {
 str_count(line["pass"], line["char"])
}
isvalid = function(line) {
 (line["count"] >= line["min"]) & (line["count"] <= line["max"]) # oui il n'y a qu'un seul &...
}

# ajout colonne valide true/false
# 2e arg apply : pour dire qu'on traite ligne par ligne
df["count"] = apply(df, 1, countchar)
df["isvalid_1"] = apply(df, 1, isvalid)

# comptage des true
# df$truc est un accesseur d'un df, quand on est à la ligne on passe en vecteur nommé (du coup c'est line["truc"])
# sum(df$isvalid_1)

# partie 2
# 2 derniers arg de substr : index début et index fin...
first_valid = function(line) {
 substr(line["pass"], line["min"], line["min"]) == line["char"]
}
second_valid = function(line) {
 substr(line["pass"], line["max"], line["max"]) == line["char"]
}

df["1_valid"] = apply(df, 1, first_valid )
df["2_valid"] = apply(df, 1, second_valid )
df["reponse"] = xor(df["1_valid"], df["2_valid"])
#sum(df$reponse)
