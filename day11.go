...
func inbound(m [][]byte, x int, y int) bool {
    h := len(m)
    w := len(m[0])
    return x < h && y < w && x >=0 && y >= 0
}

func firstseat(m [][]byte, x int, y int, xdecal int, ydecal int) byte {
    newx := x + xdecal
    newy := y + ydecal
    if inbound(m, newx, newy) {
        place := m[newx][newy]
        if place != '.' {
            return place
        } else {
            return firstseat(m, newx, newy, xdecal, ydecal)
        }
    } else {
        return '.'
    }
...
func adjacent2(m [][]byte, x int, y int) []byte {
    return []byte {firstseat(m, x, y, -1, -1), firstseat(m, x, y, -1, 0) , firstseat(m, x, y, -1, 1), 
                   firstseat(m, x, y, 0, -1), firstseat(m, x, y, 0, 1), 
                    firstseat(m, x, y, 1, -1), firstseat(m, x, y, 1, 0), firstseat(m, x, y, 1, 1)}
}

func howmany(places []byte, c byte) int {
    count := 0;
...
func howmanym(m [][]byte, c byte) int {
    count := 0;
    for _, l := range m {
        for _, p := range l {
            if p == c {
...
func nextround(m [][]byte, adjfunc func(m [][]byte, x int, y int) []byte, nb_occ int) ([][]byte, bool) {    
    h := len(m)
    w := len(m[0])
    newm := make([][]byte, len(m))
    nochange := true
    for i := 0; i < h; i++ {
        newm[i] = make([]byte, len(m[i]))
        copy(newm[i], m[i])  // copy line to new matrix
        for j := 0; j < w; j++ {
			switch m[i][j] {
			case 'L':
                // occupy if no occupied neighbours
                if howmany(adjfunc(m, i , j), '#') == 0 {
                    newm[i][j] = '#'
                    nochange = false
                }
            case '#':
                // empty if 4 or more occupied neighbours
				if howmany(adjfunc(m, i , j), '#') >= nb_occ {
                    newm[i][j] = 'L'
                    nochange = false
                }
            }
    ...
    return newm, nochange
}

func solution(s string, adjfunc func(m [][]byte, x int, y int) []byte, nb_occ int) ([][]byte, int) {
    temp := strings.Split(s,"\n")
    matrix := make([][]byte, len(temp))
    for i, v := range temp {
        matrix[i] = []byte(v)
    }
    nochange := false
    ...
    for nochange == false {
        round += 1
        matrix, nochange = nextround(matrix, adjfunc, nb_occ)
    }
    return matrix, round
}

func solution1(s string) ([][]byte, int) {    
    return solution(s, adjacent1, 4)
}

func solution2(s string) ([][]byte, int) {    
    return solution(s, adjacent2, 5)
}
