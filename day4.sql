...
create table day4 (input text not null);
insert into day4 (input) values("hgt:176cm
iyr:2013
hcl:#fffffd ecl:amb
byr:2000
eyr:2034
...
-- select SUBSTRING_INDEX(input, "\n\n", -2) from day4;
DROP PROCEDURE IF EXISTS solution1;

CREATE PROCEDURE solution1()
BEGIN
    DECLARE nb_passports INT;    
    DECLARE x  INT;
    DECLARE nb_champs  INT;
	DECLARE a_passport TEXT;
    DECLARE no_cid_present BOOL;
    DECLARE full_passport BOOL;
    DECLARE valid BOOL;
    DECLARE passport_valid_no_cid BOOL;
    CREATE TEMPORARY TABLE result (passport TEXT, nb_fields INT, contains_cid BOOL, np_credentials BOOL, res BOOL);
    -- nb passports
    SET nb_passports = (SELECT (LENGTH(input) - LENGTH(REPLACE(input, "\n\r", ""))) / 2 + 1 FROM day4);
    -- select input, REPLACE(input, "\n\r", "$") from day4;
    -- SELECT nb_passports;
    
    SET x = 0;
    loop_label:  LOOP
        IF  x >= nb_passports THEN 
			LEAVE  loop_label;
		END  IF;
        
        -- jointure des retours lignes
        SET x = x + 1;
        SET a_passport = (SELECT REPLACE(REPLACE(SUBSTRING_INDEX( SUBSTRING_INDEX(input, "\n\r", x), "\n\r", -1), "\r\n", " "), "\n", "") FROM day4);
        SET nb_champs = (SELECT (LENGTH(a_passport) - LENGTH(REPLACE(a_passport, " ", ""))) + 1);
        SET no_cid_present = (SELECT POSITION("cid:" IN a_passport) = 0);        
        SET passport_valid_no_cid = ((SELECT nb_champs = 7) AND no_cid_present);
        SET full_passport = (SELECT nb_champs = 8);
        SET valid = (full_passport OR passport_valid_no_cid);
        INSERT INTO result(passport, nb_fields, contains_cid, np_credentials, res) values(a_passport, nb_champs, no_cid_present, passport_valid_no_cid, valid);
    END LOOP;
    
    -- SELECT * FROM result WHERE res IS TRUE;
    SELECT COUNT(1) FROM result WHERE res IS TRUE;
END;

-- CALL solution1();

DROP PROCEDURE IF EXISTS solution2;
...
BEGIN
    ...
    DECLARE pid  CHAR(10);
    DECLARE hgt  CHAR(6);
    CREATE TEMPORARY TABLE result (passport TEXT, v BOOL, b TEXT, resb BOOL, i TEXT, resi BOOL, e TEXT, rese BOOL, h TEXT, resh BOOL, ec TEXT, resec BOOL, p TEXT, resp BOOL, hg TEXT, reshg BOOL, res BOOL);
    -- nb passports
    SET nb_passports = (SELECT (LENGTH(input) - LENGTH(REPLACE(input, "\n\r", ""))) / 2 + 1 FROM day4);
    -- select input, REPLACE(input, "\n\r", "$") from day4;
    -- SELECT nb_passports;
    ...
    loop_label:  LOOP
        ...
        SET valid = (full_passport OR passport_valid_no_cid);
        -- partie 2
        SET byr = (SELECT TRIM(SUBSTRING(a_passport, POSITION("byr:" IN a_passport)+4, 5)) WHERE POSITION("byr:" IN a_passport) > 0);
        SET valid = valid AND (byr >= 1920 AND byr <= 2002); -- reprise valid partie 1
        SET iyr = (SELECT TRIM(SUBSTRING(a_passport, POSITION("iyr:" IN a_passport)+4, 5)) WHERE POSITION("iyr:" IN a_passport) > 0);
        SET valid = valid AND (iyr >= 2010 AND iyr <= 2020);
        SET eyr = (SELECT TRIM(SUBSTRING(a_passport, POSITION("eyr:" IN a_passport)+4, 5)) WHERE POSITION("eyr:" IN a_passport) > 0);
        SET valid = valid AND (eyr >= 2020 AND eyr <= 2030);
        SET ecl = (SELECT SUBSTRING(a_passport, POSITION("ecl:" IN a_passport)+4, 4) WHERE POSITION("ecl:" IN a_passport) > 0);
        SET valid = valid AND (ecl REGEXP '^(amb|blu|brn|gry|grn|hzl|oth)[^[:alnum:]]?$');
        SET hcl = (SELECT SUBSTRING(a_passport, POSITION("hcl:" IN a_passport)+4, 8) WHERE POSITION("hcl:" IN a_passport) > 0);        
        SET valid = valid AND (hcl REGEXP '^#[0-9a-f]{6}[^[:alnum:]]?$');
        SET pid = (SELECT SUBSTRING(a_passport, POSITION("pid:" IN a_passport)+4, 10) WHERE POSITION("pid:" IN a_passport) > 0);
        SET valid = valid AND (pid REGEXP '^[0-9]{9}[^[:alnum:]]?$');
        SET hgt = (SELECT SUBSTRING(a_passport, POSITION("hgt:" IN a_passport)+4, 6) WHERE POSITION("hgt:" IN a_passport) > 0);
        SET valid = valid AND (hgt REGEXP '^([1][5-8][0-9]cm|19[0-3]cm)[^[:alnum:]]?$|^(59in|6[0-9]in|7[0-6]in)[^[:alnum:]]?.?$'); -- regexp pourrie...
        INSERT INTO result(passport, v, b, resb, i, resi, e, rese, h, resh, ec, resec, p, resp, hg, reshg, res) 
        VALUES (a_passport, (full_passport OR passport_valid_no_cid), byr, (byr >= 1920 AND byr <= 2002), iyr, (iyr >= 2010 AND iyr <= 2020), eyr, (eyr >= 2020 AND eyr <= 2030), 
                ecl, (ecl REGEXP '^(amb|blu|brn|gry|grn|hzl|oth)[^[:alnum:]]?$'), hcl, (hcl REGEXP '^#[0-9a-f]{6}[^[:alnum:]]?$'), 
                pid, (pid REGEXP '^[0-9]{9}[^[:alnum:]]?$'), hgt, (hgt REGEXP '^([1][5-8][0-9]cm|19[0-3]cm)[^[:alnum:]]?$|^(59in|6[0-9]in|7[0-6]in)[^[:alnum:]]?.?$'), valid);
    END LOOP;
    
    SELECT COUNT(1) FROM result WHERE res IS TRUE;
    -- SELECT * FROM result where v is True;    
END;
