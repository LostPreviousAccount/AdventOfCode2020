...
for rule in arr_input:
    elem = rule.replace("bags","bag").split("contain")
    key = elem[0].strip().replace(" ","_") 
    values = elem[1].replace(".","").split(",")
    for v in values:
        contained = v.strip().replace(" ","_")
        print(f'contains({key},{contained[0].replace("n","0")},{contained[2:].replace("_other","other")}).')
