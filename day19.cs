...
aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba";
        ...
        public static Tuple<string,string[]> Replace(Dictionary<string, string> rules, KeyValuePair<string,string[]> kv, bool part2) {
            var val=kv.Value;
            foreach(var rule in rules) {
                for(int i=0; i<val.Count(); i++) {
                    if(val[i].Equals(rule.Key)) {
                        val[i] = rule.Value;
                    }
                }
            }
            if (IsFinalRule(val)) {
                val = new string[] { "(" + string.Join(String.Empty, val).Replace("\"","") + ")" }; // convert to regexp
                if (part2 && kv.Key.Equals("8")) { //8: 42 == > 8: 42 | 42 8 ==> + (because it becomes at least one 42)
                    val[0] +="+";
                }
            }
            return Tuple.Create(kv.Key, val);
        }
        
        public static bool IsFinalRule(string[] str) { // should not contain any digit
            return str.Where(s => Regex.IsMatch(s, @"\d+")).Count() == 0;
        }
        
        public static string RulesToRegex(string allRules, bool part2) {
            Dictionary<string, string[]> rules = allRules
                .Split(Environment.NewLine)
                .Select(r => r.Split(':'))
                .ToDictionary( t => t[0], t => t[1].Trim().Split(" ") );
            if (part2) {
                rules["11"] = new string[] { "42","31","|","42","42","31","31","|","42","42","42","31","31","31","|","42","42","42","42","31","31","31","31","|","42","42","42","42","42","31","31","31","31","31"};//11: 42 31 ==> 11: 42 31 | 42 11 31
                // hack till it passes... (result does not change anymore)
            }
            while (rules.Keys.Count()>1) {
                Dictionary<string, string> darules = //final rules...
                    (from r in rules
                    where IsFinalRule(r.Value)
                    select r).ToDictionary( kv => kv.Key, kv => kv.Value[0]);
                rules = rules
                    .Where(r => !darules.Keys.Contains(r.Key))
                    .Select(r => Replace(darules, r, part2)) // replace finale rules in other rules using them
                    .ToDictionary( kv => kv.Item1, kv => kv.Item2);
            }
            //rules.Select(i => $"{i.Key}: {string.Join(String.Empty, i.Value)}").ToList().ForEach(Console.WriteLine);
            return $"^{rules.Values.First()[0]}$"; // regexp to match beggining and end of string (First and [0] because of dict and string[])
        }
        
        public static int Solution1(string input) {
            //split input by double line break
            string[] parts = input.Replace(Environment.NewLine+Environment.NewLine,"@").Split('@');
            // rules = first part
            string rules = RulesToRegex(parts[0], false);
            // match with the regexp rules
            var matchCount = parts[1].Split(Environment.NewLine)
                    .Where(l => Regex.IsMatch(l, rules))
                    .Select(l => l)
                    .Count();
            return matchCount;
        }
        ...
        public static int Solution2(string input) {
            string[] parts = input.Replace(Environment.NewLine+Environment.NewLine,"@").Split('@');
            string rules = RulesToRegex(parts[0], true);
            var matchCount = parts[1].Split(Environment.NewLine)
                    .Where(l => Regex.IsMatch(l, rules))
                    .Select(l => l)
                    .Count();
